...
const MonthlyDashboard: React.FC = () => {
  ...
  const chartData = useMemo(() => {
    ...
    // Group by date
    const grouped = envSubmissions.reduce((acc: any, sub) => {
      const date = sub.submittedAt.split('T')[0];
      if (!acc[date]) acc[date] = { date, temp: [], humidity: [] };
      
      const tValue = parseFloat(sub.data['q3']);
      const hValue = parseFloat(sub.data['q5'] || sub.data['q6']);
    ...
    return Object.values(grouped).map((day: any) => ({
      date: day.date,
      avgTemp: day.temp.length > 0 ? (day.temp.reduce((a: any, b: any) => a + b, 0) / day.temp.length).toFixed(1) : null,
      avgHumidity: day.humidity.length > 0 ? (day.humidity.reduce((a: any, b: any) => a + b, 0) / day.humidity.length).toFixed(1) : null,
