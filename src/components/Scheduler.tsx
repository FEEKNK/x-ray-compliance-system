...
import type { Schedule, Shift, DynamicForm, User as UserType } from '../types';
            ...
            <div className="divide-y divide-gray-50">
              {schedules.map(s => (
                <div key={s.id} className="p-6 flex items-center justify-between">
                   ...
                   <button onClick={() => deleteSchedule(s.id)} className="text-red-400 p-2 hover:bg-red-50 rounded-lg transition-all"><X size={16} /></button>
                </div>
              ))}
            </div>
          </div>
