...
export const usePublicUsers = () =>
  useQuery({ queryKey: ['users', 'public'], queryFn: api.users.getPublic });
...
export const useUpdateUser = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (user: Partial<User> & { id: string }) => api.users.update(user.id, user),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['users'] }),
  });
};
...
export const useForms = () =>
  useQuery({ queryKey: ['forms'], queryFn: api.forms.getAll });

export const useAddForm = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: api.forms.create,
...
export const useUpdateForm = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (form: DynamicForm) => api.forms.update(form.id, form),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['forms'] }),
  });
};
...
export const useBulkDeleteSchedules = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: api.schedules.bulkDelete,
    onSuccess: () => qc.invalidateQueries({ queryKey: ['schedules'] }),
  });
};
...
export const useAddSubmission = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ['schedules'] }); // Adding submission completes a schedule
    },
  });
};
...
export const useDeleteSubmission = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['submissions'] });
      qc.invalidateQueries({ queryKey: ['schedules'] }); // Deleting submission reverts schedule
    },
  });
};
...
export const useBundles = () =>
  useQuery({ queryKey: ['bundles'], queryFn: api.bundles.getAll });
...
export const useUpdateBundle = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (bundle: ProtocolBundle) => api.bundles.update(bundle.id, bundle),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['bundles'] }),
  });
};
...
export const useAlerts = () =>
  useQuery({ queryKey: ['alerts'], queryFn: api.alerts.getAll });
...
export const useConfig = () =>
  useQuery({ queryKey: ['config'], queryFn: api.config.get });

export const useUpdateSettings = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: api.config.updateSettings,
...
export const useAddAnnouncement = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: api.config.addAnnouncement,
    onSuccess: () => qc.invalidateQueries({ queryKey: ['config'] }),
  });
};
