...
// Valid alert types
const VALID_ALERT_TYPES = ['Missed Task', 'Critical Failure', 'SLA Warning', 'System'] as const;
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!VALID_ALERT_TYPES.includes(type)) {
      return res.status(400).json({ error: `Invalid alert type. Must be one of: ${VALID_ALERT_TYPES.join(', ')}` });
    }
    if (typeof message !== 'string' || message.length > 1000) {
      return res.status(400).json({ error: 'Message must be a string with max 1000 characters' });
    }
    ...
    if (type === 'Missed Task' && staffId) {
      ...
      (async () => {
        ...
        if (staff && staff.email && isValidEmail(staff.email)) {
          ...
          logger.info(`[Alert Email] ✅ Missed task email sent to ${staff.email}`);
        } else if (staff) {
          logger.warn(`[Alert Email] ⚠️ Skipping email for ${staff.name} — invalid email: "${staff.email}"`);
        }
      })().catch(e => {
